    ...
    <section aria-label="Examples and constraints">
      {examples.map((example, index) => (
        <div className="problem-example" key={`${example.input}-${index}`}>
          ...
          <pre>{`Input:\n${example.input}\n\nOutput:\n${example.output}`}</pre>
          {example.explanation && <p><RichTextRenderer value={example.explanation} /></p>}
        </div>
      ))}
      {constraints.length > 0 && (
        <div>
          <h3>Constraints</h3>
          <ul>{constraints.map((constraint, index) => <li key={index}><RichTextRenderer value={constraint} /></li>)}</ul>
        </div>
      )}
    </section>
  );
